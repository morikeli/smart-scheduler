...
def user_notifications(request):
    ...
    if request.user.is_anonymous is False:
        try:
            lecturer_notifications = Notification.objects.filter(scheduled_lecture__lecturer=request.user.faculty) if str(request.user) == str(request.user.faculty) else []
            total_lec_notifications = lecturer_notifications.count() if len(lecturer_notifications) > 0 else total_stud_notifications

        except:
            student_notifications = Notification.objects.filter(scheduled_lecture__student=request.user.student) if str(request.user) == str(request.user.student) else []
